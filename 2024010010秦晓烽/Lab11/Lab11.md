...
| :--- | :------- |
| 签名 RSA 密钥长度（位） |2048 |
| 签名私钥 `signature_private_key.pem` 文件大小（字节） |1704 |
| 签名公钥 `signature_public_key.pem` 文件大小（字节） | 460|
| 签名公钥指数 e 的值 | 65537 (0x10001)|
| 签名公钥模数 N 的前 16 位十六进制 | 009b26a927592978|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） |这是一条重要消息，需要签名保护。发送者：樊子逸，学号：2024010025 |
| 密文文件 encrypted_message.bin 大小（字节） |256 |
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |978318dc5731e79dfe8bc8a834441766ce28fd33bc603b033baecfed0f83cd31 |
...
> 答：身份认证（Authentication）：验证消息确实来自持有私钥的特定发送者。在本实验中，我使用 signature_private_key.pem 对密文签名，接收方使用对应的 signature_public_key.pem 验证。当终端输出 Verified OK 时，就数学上证明了该密文必定是由我（私钥持有者）签署的，因为只有私钥才能生成通过公钥验证的签名。
> 完整性保护（Integrity）：确保消息在传输过程中未被篡改。实验中，当我向原始密文 encrypted_message.bin 末尾追加了 1 个字节的 'x' 形成篡改文件后，原本有效的签名立即验证失败（预期输出 Verification failure）。这说明签名与密文的每一个比特都紧密绑定，任何微小的改动都会导致哈希值变化，从而使签名失效。
> 不可否认性（Non-repudiation）：发送者无法事后否认发送过该消息。因为签名私钥具有唯一性且仅为发送者持有，接收者验证通过后，可将签名作为“数字证据”提交给第三方（如仲裁机构）。在本实验流程中，我无法否认对 encrypted_message.bin 的签名，因为该签名只能由我的私钥生成。
...
> 答：效率问题：RSA 非对称加密运算速度极慢（涉及大整数模幂运算）。本实验中 encrypted_message.bin 虽小（256 字节），但实际应用中消息可能达 GB 级。哈希函数（如 SHA-256）计算极快，能将任意长度的数据压缩为固定的 256 位（32 字节），签名运算量剧减。
> 输入长度限制：RSA 算法本身是数学运算，能处理的数据块长度受限于模数N 的长度（本实验为 2048 位 = 256 字节）。如果直接对密文进行裸 RSA 运算，则无法处理大于 256 字节的数据。先哈希后签名彻底规避了这一限制。
> 标准化与安全性：对固定长度的哈希值签名使算法接口统一。此外，哈希函数的抗碰撞性为签名提供了额外的安全屏障——即使攻击者能破解 RSA 私钥（极端情况），也无法轻易构造出具有特定哈希值的恶意消息。
...
> 答：signature.bin 的大小为 256 字节。这是因为 RSA 签名的数学本质是计算 σ=H(m) dmodN。其中，模数 N 的长度决定了运算结果 σσ 的取值范围（0≤σ<N0≤σ<N）。我的密钥长度为 2048 位（即 2048 / 8 = 256 字节），所以签名结果必然占用 256 字节的存储空间。如果使用 4096 位的 RSA 密钥，签名长度的计算公式为 4096/8=4096/8= 512 字节。因此，签名长度与密钥模数长度（以位为单位除以 8）严格成正比。
...
> 答：抗原像性（Preimage Resistance，单向性）：给定哈希值 h，难以找到原像 m使得 H(m)=hH(m)=h。若被攻破，攻击者可从截获的签名 σ 中反推出密文哈希值，进而尝试碰撞出原始密文，破坏保密性。
> 抗第二原像性（Second Preimage Resistance）：给定消息 m1，难以找到另一个不同的 m2使得 H(m1)=H(m2)H(m 1​)=H(m 2)。若被攻破，攻击者可以在不改变哈希值的情况下替换消息内容，从而将同一个签名“移植”到另一个不同的密文上。
> 抗碰撞性（Collision Resistance）：难以找到任意两个不同的消息 m1 和 m2使得二者哈希相同。若被攻破，攻击者可以预先准备一份“正常文件”和一份“恶意文件”，诱骗签名者签署正常文件，随后将签名移植到恶意文件上，造成严重安全漏洞。
...
> 答：SHA-1 已不再被推荐用于数字签名，因为其抗碰撞性已被实际攻破。2017 年，Google 与 CWI Amsterdam 联合公布了首个实际的 SHA-1 碰撞攻击（SHAttered 攻击），攻击者可以在有限计算资源内构造出两个内容不同但 SHA-1 哈希值完全相同的 PDF 文件。
> 对数字签名的影响：攻击者可利用这一漏洞，准备文件 A（内容合法、无害）和文件 B（内容恶意）。由于二者 SHA-1 相同，签名者若对文件 A 签名，该签名同样能通过文件 B 的验证。受害者（如软件下载者）验证签名通过，便误以为文件 B 是官方发布的正版文件。这正是为什么现代签名标准（如本实验）强制要求使用 SHA-256 或更强的哈希算法。
...
> 答：签名私钥 (signature_private_key.pem)：用于生成数字签名（-sign 参数）。此密钥必须绝对保密，一旦泄露，任何人都可以伪造我的签名。
> 签名公钥 (signature_public_key.pem)：用于验证数字签名（-verify 参数）。此密钥可以公开，任何人都能用它来确认消息来源。
> 加密私钥 (encryption_private_key.pem)：用于解密接收到的密文（-decrypt 参数）。此密钥必须保密，确保只有我能阅读发给我的消息。
> 加密公钥 (encryption_public_key.pem)：用于加密发送给接收方的消息（-encrypt 参数）。此密钥可以公开。
> 攻击者获得公钥后：
> 能做的：使用公钥加密发给我的消息、使用签名公钥验证我的数字签名。
> 不能做的：解密密文（缺少私钥指数 d）、伪造我的数字签名（缺少签名私钥 d）。因此，公钥的公开不会威胁系统安全性。
...
> 答：消息加密（机密性）：使用接收方的公钥加密，接收方用私钥解密。目的是防止消息内容在传输过程中被第三方窃听。在本实验中，encrypted_message.bin 保护了明文内容 message.txt。
> 数字签名（身份认证与完整性）：使用发送方的私钥签名，接收方用发送方的公钥验证。目的是证明消息来源可靠且未被篡改。
> 不能简单地用加密代替签名：加密使用公钥（公开的），任何人只要有接收方的公钥都能加密消息，因此加密无法证明发送者的身份。而签名使用私钥（唯一的），只有发送者才能生成有效签名。两者解决的问题正交，在实际安全系统中往往缺一不可（如本实验中，加密保护内容，签名保护来源）。
...
> 答：不可否认性是指发送方无法否认自己发送过该消息，这在法律和审计层面具有重要意义。
> 数字签名能提供不可否认性，是因为它基于非对称密钥体系：签名私钥仅有发送方一人持有。当验证方（或第三方仲裁机构）用发送方的公钥验证签名通过时，在数学上等同于证明了该签名必然是由私钥生成的。发送方无法将责任推卸给接收方或攻击者。
> 相比之下，消息认证码（MAC） 使用对称密钥，发送方和接收方共享同一个密钥。因此，接收方完全有能力自己生成一个合法的 MAC 并声称消息来自发送方。由于无法区分 MAC 是由发送方还是接收方生成的，MAC 不具备向第三方证明来源的不可否认性。
...
> 答：软件发布与更新（如 Microsoft、Adobe 官方下载站）：开发者使用私钥对安装包或二进制文件签名。用户下载后验证签名，确保软件确实来自官方且未被植入病毒或后门（解决了本实验中的完整性与来源问题）。
> 电子邮件安全（S/MIME 或 PGP/GPG）：发件人对邮件内容或附件进行签名，收件人验证签名以确认邮件地址未被伪造（防止 CEO 诈骗/BEC 攻击）。
> Git 代码提交签名：开发者（如 Linux 内核维护者）使用 GPG 私钥对代码 Commit 或 Tag 签名，确保代码仓库中的提交确实来自可信的开发者，而非冒用 GitHub 账号的黑客。
...
> 答：数字签名本身无法证明签名的时间。因为私钥在有效期内随时可用，持有者可以随意对任意历史文件进行签名，并声称是在某个过去的时间点签署的。
> 为了解决这一问题，引入了可信时间戳服务（TSA，Time Stamping Authority）。流程如下：
> 发送方计算文件的哈希值，将其发送给 TSA。
> TSA 将“文件哈希值 + 当前精确时间”拼接后，使用 TSA 自己的私钥进行数字签名，生成时间戳凭证并返回给发送方。
> 接收方验证 TSA 的签名，即可确认该文件在 TSA 盖章的那个时间点之前已经存在且未被篡改。这解决了签名时间的争议，广泛用于专利证明、电子合同签署和长期档案保全。
...
> 答：本实验选择 2048 位 RSA 密钥，是基于安全性与性能的权衡：
> 1024 位：已不安全。随着计算能力提升（尤其是云计算和大规模并行计算），1024 位 RSA 已被实际攻破（建议彻底弃用），不再适用于任何安全敏感场景。
> 2048 位：当前行业标准。NIST（美国国家标准与技术研究院）建议 2048 位密钥有效期至少到 2030 年。其在现有计算能力下提供了足够的安全边际（破解需数十亿年），同时生成密钥、签名和解密的速度在可接受范围内。这也是本实验采用 2048 位的原因。
> 4096 位：极高安全等级。虽然安全性远高于 2048 位，但生成密钥和进行签名/解密的运算时间显著增加（约慢 6~10 倍），且签名长度翻倍至 512 字节。通常仅用于根证书颁发机构（Root CA）私钥或需要长期保护（数十年）的顶级敏感数据，日常通信和一般企业级应用采用 2048 位已足够。
...
> 答：“签名的签名”（多重签名或嵌套签名）具有实际意义。
> 在复杂的业务流程中，一份文件往往需要经过多个角色的审批。例如：
> 经理 Alice 签署了项目计划书。
> 财务总监 Bob 对“Alice 的签名 + 计划书文件”整体进行二次签名。
> 最终 CEO 收到文件时，可以依次验证 Alice 的签名（确认经理已审阅）和 Bob 的签名（确认财务已审阅），从而证明该文件经过了完整的、不可否认的审批链条。
> 这种机制在法律合同审核、企业内部财务授权（如需要多级审批的转账）、以及区块链多重签名钱包（如 2-of-3 多签地址）中广泛应用。每一层签名都绑定其下层的数据，任何一环的篡改都会导致最终验证失败，确保了流程的严肃性与不可抵赖性。
